...
<body>
	<div class="topnav" >
		...
		$email=$_SESSION['id'];
		$profile_id=$_GET['req_id'];
		$sql="SELECT * FROM profile WHERE email='$profile_id'";
		...
		#echo "<h2>Friends</h2>";
			...
			$sqlfl="SELECT COUNT(email) as count FROM profile WHERE (email in (SELECT friend_id FROM friends_list WHERE user_id='$profile_id') or email in(select user_id FROM friends_list WHERE friend_id='$profile_id'))";
	...
	<div class="timeline" align="center">
		...
		<div class="news">
			<?php
			$sql="SELECT * FROM stories WHERE user_id='$profile_id'";
			$result=$conn->query($sql);
			#echo $result;
			if ($result->num_rows > 0) 
			{
			...
			if ($result1->num_rows > 0) 
			{
				...
				echo "<caption>caption: <b><i>$caption</i></b> </caption><br><br>";
				echo "</div>";
				}
			}
			 } 
